...
func get(image):
	...
	for i in paths.size():
		paths[i].vertices = optimize_path(paths[i].vertices)
	t2 = OS.get_ticks_msec()
	print("optimization time")
	print(t2 - t1)
	t1 = OS.get_ticks_msec()
	for i in paths.size():
		paths[i].vertices = fix_path(image, paths[i].vertices)
	t2 = OS.get_ticks_msec()
	print("fixing time")
	print(t2 - t1)
	t1 = OS.get_ticks_msec()
	...
	var export_paths = []
	for i in paths.size():
		export_paths.append(paths[i].vertices)
	config.set_value("paths", "paths", export_paths)
...
func fix_path(image, path):
	var new_path = []
	for i in path:
		var information = get_pixel_information(image, [], i.x, i.y)
		if information.u_l && ! information.u_r && ! information.d_l && ! information.d_r:
			i += Vector2(-0.1, -0.1)
		if ! information.u_l && information.u_r && ! information.d_l && ! information.d_r:
			i += Vector2(0.1, -0.1)
